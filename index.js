...
function buildNameRegex(patternList) {
    ...
    try { return new RegExp(b, f) } catch (err) { return console.warn("buildNameRegex compile failed:", err), null }
}
function buildSpeakerRegex(patternList) {
    ...
    const b = `(?:^|\\n)\\s*(${p.join('|')})\\s*[:;,]\\s*`;
    const f = computeFlagsFromEntries(e, true);
    try { return new RegExp(b, f) } catch (err) { return console.warn("buildSpeakerRegex compile failed:", err), null }
}
function buildVocativeRegex(patternList) {
    ...
    const b = `(?:["“'\\s])(${p.join('|')})[,.!?]`;
    const f = computeFlagsFromEntries(e, true);
    try { return new RegExp(b, f) } catch (err) { return console.warn("buildVocativeRegex compile failed:", err), null }
}
...
function buildAttributionRegex(patternList, verbList) {
    ...
    // Pattern for: "Quote," Character said.
    const postQuote = `(?:["“”][^"“”]*["“”])\\s*[,:;\\-–—]?\\s*(${names})${optionalMiddleName}\\s+(?:${verbs})\\b`;
    // Pattern for: Character said, "Quote."
    const preQuote = `\\b(${names})${optionalMiddleName}\\s+(?:${verbs})\\s*[:,]?\\s*["“”]`;

    const body = `(?:${postQuote})|(?:${preQuote})`;
    const flags = computeFlagsFromEntries(entries, true);
    try { return new RegExp(body, flags) } catch (err) { console.warn("buildAttributionRegex compile failed:", err); return null }
}
...
function isIndexInsideQuotesRanges(ranges, idx) {
    for (const [a, b] of ranges) if (idx > a && idx < b) return !0;
    return !1
}
function findMatches(combined, regex, quoteRanges, searchInsideQuotes = !1) {
    if (!combined || !regex) return [];
    const flags = regex.flags.includes("g") ? regex.flags : regex.flags + "g", re = new RegExp(regex.source, flags), results = [];
    let m;
    for (; (m = re.exec(combined)) !== null;) {
        const idx = m.index || 0;
        (searchInsideQuotes || !isIndexInsideQuotesRanges(quoteRanges, idx)) && results.push({ match: m[0], groups: m.slice(1), index: idx }), re.lastIndex === m.index && re.lastIndex++
    }
    return results
}
...
jQuery(async () => {
    ...
    function testRegexPattern() {
        ...
        const tempRegexes = { speakerRegex: buildSpeakerRegex(effectivePatterns), attributionRegex: buildAttributionRegex(effectivePatterns, tempProfile.attributionVerbs), actionRegex: buildActionRegex(effectivePatterns, tempProfile.actionVerbs), vocativeRegex: buildVocativeRegex(effectivePatterns), nameRegex: buildNameRegex(effectivePatterns) };
    ...
    registerSlashCommand("scene", (args) => {
        ...
        const tempProfile = saveCurrentProfileData();
        if (!tempProfile) {
            toastr.error("Could not load current profile settings for analysis.");
            return;
        }
        const lowerIgnored = (tempProfile.ignorePatterns || []).map(p => String(p).trim().toLowerCase());
        const effectivePatterns = (tempProfile.patterns || []).filter(p => !lowerIgnored.includes(String(p).trim().toLowerCase()));
        ...
        if (debugMode) {
            ...
            toastr.info(debugString, "Scene Analysis Debug", { timeOut: 15000, "escapeHtml": false });
            return;
        }
    ...
    }, ["debug"], "Analyzes the last AI message to determine the primary characters. Use 'debug' to see scores.", true);
