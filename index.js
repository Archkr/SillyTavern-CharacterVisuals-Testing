...
const DEFAULTS = {
    ...
    focusLock: '', // Changed from object to simple string for easier management
};
...
function calculateCharacterFocusScores(text, profile, regexes) {
    ...
    const scores = {};
    const points = { speaker: 3, attribution: 3, action: 2, vocative: 1, possessive: 1, name: 1 };
    allMatches.forEach(match => {
        const normalizedName = normalizeCostumeName(match.name);
        if (!scores[normalizedName]) { scores[normalizedName] = 0; }
...
jQuery(async () => {
    ...
    // FIX: Wait for UI to be injected before wiring, for robustness.
    ...
    function recompileRegexes() {
        try {
            ...
            const attributionVerbs = String(profile.attributionVerbs || '').replace(/\s*\|\s*/g, '|');
            const actionVerbs = String(profile.actionVerbs || '').replace(/\s*\|\s*/g, '|');
    ...
    // FIX: Added dedicated function to update Focus Lock UI state
    ...
    function loadProfile(profileName) {
        ...
        $("#cs-detection-bias-value").text(profile.detectionBias || PROFILE_DEFAULTS.detectionBias); // FIX: Ensure value is displayed on load
        ...
        updateFocusLockUI(); // FIX: Update lock UI on profile change
        recompileRegexes();
    }
    ...
    function tryWireUI() {
        ...
        // FIX: Replaced old handler with fully functional Focus Lock logic
        $("#cs-focus-lock-toggle").off('click.cs').on("click.cs", async () => {
            if (settings.focusLock) {
                ...
                if (selectedChar) {
                    settings.focusLock = selectedChar;
        ...
        // FIX: Replaced old handler with one that provides real-time feedback
        ...
        $("#cs-save").off('click.cs').on("click.cs", () => {
            ...
            if(profileData) {
                ...
                recompileRegexes();
                updateFocusLockUI(); // FIX: Update lock dropdown after saving new patterns
                persistSettings();
            }
        });
        ...
        $("#cs-profile-save").off('click.cs').on("click.cs", () => {
            ...
            populateProfileDropdown();
            updateFocusLockUI(); // FIX: Also update lock dropdown when duplicating/creating profile
            $("#cs-error").text("").hide();
            persistSettings();
        });
    ...
    registerSlashCommand("scene", 
        (args) => {
            ...
            if (!lastMessage) { toastr.warning("Could not find the last AI message to analyze."); return; }
            const tempProfile = {
                patterns: $("#cs-patterns").val().split(/\r?\n/).map(s => s.trim()).filter(Boolean),
                ignorePatterns: $("#cs-ignore-patterns").val().split(/\r?\n/).map(s => s.trim()).filter(Boolean),
                attributionVerbs: $("#cs-attribution-verbs").val().trim().replace(/\s*\n\s*/g, '|'),
            ...
            } catch (e) { toastr.error(`Failed to build patterns for analysis: ${e.message}`); return; }
            const scores = calculateCharacterFocusScores(lastMessage.mes, tempProfile, tempRegexes);
            const sortedScores = Object.entries(scores).sort((a, b) => b[1] - a[1]);
            if (sortedScores.length === 0) { toastr.info("No primary characters were detected in the last message."); return; }
            if (debugMode) {
                let debugString = "<strong>Scene Debug Scores:</strong><br>";
                sortedScores.forEach(([name, score]) => { debugString += `${name}: ${score}<br>`; });
